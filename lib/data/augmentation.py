...
class Augmenter2D(object):
    ...
    def add_noise(self, motion_2d):
        ...
        if "uniform_range" in self.noise.keys():
            ...
            uniform_range = 0.06
        motion_2d = motion_2d[:,:,:,:2] # Remove confidence from motion if present
        ...
        sel = torch.rand((batch_size, self.num_Kframes, num_joints, 1)) # Randomly select joints to add noise to
        gaussian_sample = (torch.randn(batch_size, self.num_Kframes, num_joints, 2) * std + mean) # Gaussian noise
        ...
        gaussian_sample = gaussian_sample.to(motion_2d.device) # Apply gaussian noise to motion data
        uniform_sample = uniform_sample.to(motion_2d.device) # Apply uniform noise to motion data
    ...
    def add_mask(self, x):
        ...
        '''
        N,T,J,C = x.shape
        mask = torch.rand(N,T,J,1, dtype=x.dtype, device=x.device) > self.mask_ratio # Drop joints
        mask_T = torch.rand(1,T,1,1, dtype=x.dtype, device=x.device) > self.mask_T_ratio # Drop frames
